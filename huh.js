(() => {
	...
	const halfRotation = MAX_ROTATION / 2;
	const rotate = (event) => {
		const rotationY =
			MAX_ROTATION * (1 - event.clientY / window.innerHeight) - halfRotation;
		const rotationX =
			(MAX_ROTATION * event.clientX) / window.innerWidth - halfRotation;
		body.style.transform = `rotateX(${rotationY}deg) rotateY(${rotationX}deg)`;
	};
	// const animRotate = (e) => re
	document.addEventListener('mousemove', (e) =>
		requestAnimationFrame(rotate.bind(null, e))
	);
